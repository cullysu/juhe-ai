...
export const MD_VENDOR_CODE = 'md'
...
export const MD_OPENAI_V1_PROFILE_ID = 'profile_md_openai_v1'
...
export function isOpenAICompatibleProviderCode(value: unknown): boolean {
  ...
  return normalized === OPENAI_COMPATIBLE_PROVIDER_CODE || normalized === GPT_VENDOR_CODE || normalized === MD_VENDOR_CODE
}

export function preferredDefaultProvider(providers: ProviderDefinition[]): ProviderDefinition | undefined {
  const enabledProviders = providers.filter((provider) => provider.enabled)
  const openAICompatibleProviders = enabledProviders.filter((provider) => isOpenAIProtocolProvider(provider))
  return enabledProviders.find((provider) => normalizeProviderToken(provider.code) === OPENAI_COMPATIBLE_PROVIDER_CODE)
    ?? openAICompatibleProviders.find((provider) => normalizeProviderToken(provider.code) !== GPT_VENDOR_CODE)
